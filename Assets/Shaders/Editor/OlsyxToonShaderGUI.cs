...
public class OlsyxToonShaderGUI : OlsyxShaderGUI {
    ...
    void CelShadingSection() {
        ...
        int pixelsBetweenColors = 15;

        if (!useShadingRamp) {
            GUILayout.Space(pixelsBetweenColors);

            ColorAndSliderInLine("_SpecularTint", "Specular Tint", "_Smoothness", "Threshold");
            GUILayout.Space(pixelsBetweenColors);

            ColorAndSliderInLine("_DiffuseColor", "Diffuse Color", "_DiffuseThreshold", "Threshold");
            GUILayout.Space(pixelsBetweenColors);

            ColorAndSliderInLine("_UnlitColor", "Unlit Color", "_UnlitThreshold", "Threshold");
            GUILayout.Space(pixelsBetweenColors);
        }
    }

    void ShadingRampSection() {
        MaterialProperty granularity = FindProperty("_Granularity", properties);
        // editor.RangeProperty(granularity, "Sharpness");

        MaterialProperty shadingMap = FindProperty("_ShadingRamp", properties);
        Texture tex = shadingMap.textureValue;

        EditorGUI.BeginChangeCheck();
        editor.TexturePropertySingleLine(MakeLabel(shadingMap, "Shading Ramp"), shadingMap, granularity);

        if (EditorGUI.EndChangeCheck()) {
            SetKeyword("_USE_SHADING_RAMP", shadingMap.textureValue);
            useShadingRamp = IsKeywordEnabled("_USE_SHADING_RAMP");
        }
